#!/usr/bin/env bash
# Termux bootstrap: libbash.so in jniLibs + bootstrap.bin asset (gzip tar of prefix + xkb).
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
JNILIBS="$ROOT/app/src/main/jniLibs/arm64-v8a"
ASSET="$ROOT/app/src/main/assets/bootstrap.bin"
MARKER="$ROOT/app/src/main/assets/.bootstrap_prepared_v2"
ARCH="aarch64"
...
if [[ -f "$MARKER" && -f "$ASSET" && -f "$JNILIBS/libbash.so" ]]; then
...
touch "$MARKER"
echo "==> Wrote $ASSET (with XKB) and $JNILIBS/libbash.so"
ls -lh "$ASSET" "$JNILIBS/libbash.so"
